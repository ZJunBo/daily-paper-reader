...
title_zh: UNCERTAINTY-LINE：大语言模型的长度不变不确定性估计
...
score: 9.0
evidence: 长度不变的不确定性估计方法
tldr: 现有基于token概率的不确定性估计存在输出长度偏差，即使长度归一化也无法消除。本文提出UNCERTAINTY-LINE，通过对不确定性分数与输出长度进行回归来去除长度影响，实现长度不变的不确定性估计。实验表明该方法有效消除了长度偏差，提高了不确定性的可靠性。有助于更公正地评估LLM输出。
...
motivation: 消除输出长度对不确定性估计的偏差。
method: 将不确定性分数对输出长度进行回归，用残差作为无偏估计。
result: 成功去除了长度偏差，不确定性估计更准确。
conclusion: 为LLM不确定性量化提供了一种简单有效的去偏方法。
---
