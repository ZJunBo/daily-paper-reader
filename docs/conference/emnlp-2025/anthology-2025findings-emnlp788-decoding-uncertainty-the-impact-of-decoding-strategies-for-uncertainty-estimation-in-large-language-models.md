...
score: 9.0
evidence: 解码策略对不确定性估计的影响
tldr: 解码策略改变输出概率分布，从而影响不确定性估计。本文系统评估了多种解码策略对LLM不确定性估计的影响，发现Contrastive Search在偏好对齐的LLM上平均表现更优，但在仅经过SFT的模型上效果不一。该发现指导不同场景下解码策略的选择以优化不确定性估计。
...
motivation: 解码策略对不确定性估计的影响鲜有研究。
method: 对比多种解码策略在不同对齐程度的LLM上的不确定性估计效果。
result: Contrastive Search在对齐模型上表现更好。
conclusion: 选择解码策略时需考虑对齐状态以优化不确定性。
---
