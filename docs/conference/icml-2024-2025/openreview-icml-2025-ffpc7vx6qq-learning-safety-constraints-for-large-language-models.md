...
tags: ["query:smd"]
score: 5.0
evidence: 学习安全约束以检测和纠正不安全输出
tldr: 该论文提出安全多面体（SaP）方法，在LLM表示空间中学习并强制执行多重安全约束。通过识别安全与不安全区域，实现对不安全输出的检测和几何纠正。实验表明，该方法在不影响模型能力的前提下有效检测不道德输入，可作为一种后处理防御机制，为抵御有害微调攻击提供新思路。
...
motivation: LLM存在有害输出和对抗攻击的安全风险，需要不修改权重即可强制执行安全约束的方法。
method: 提出安全多面体（SaP），利用表示空间中的多面体面识别安全/不安全区域，通过几何引导纠正不安全输出。
result: 在多个LLM上实验证明，SaP能有效检测不道德输入并纠正不安全输出，同时保持模型原有能力。
conclusion: SaP作为后处理安全约束方法，为LLM的安全防御提供了轻量级、可迁移的解决方案。
---
